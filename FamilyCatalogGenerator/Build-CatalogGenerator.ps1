...
<#
.SYNOPSIS
    Builds the Family Catalog Generator into a standalone executable.
...
$sourceScript = Join-Path $scriptPath "Generate-FamilyCatalog.ps1"
$outputExe = Join-Path $scriptPath "dist\FamilyCatalogGenerator.exe"
$distFolder = Join-Path $scriptPath "dist"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host " GSADUs Family Catalog Generator Builder" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Invoke-PS2EXE -InputFile $sourceScript `
                  ...
                  -Title "GSADUs Family Catalog Generator" `
                  -Description "Generates CSV catalogs for Revit family files" `
                  -Company "GSADUs" `
                  -Product "Family Catalog Generator" `
                  -Version "1.0.0.0" `
    ...
    if (Test-Path $outputExe) {
        ...
        $batchFile = Join-Path $scriptPath "FamilyCatalogGenerator.bat"
        $scriptCopy = Join-Path $distFolder "Generate-FamilyCatalog.ps1"
        $batchCopy = Join-Path $distFolder "FamilyCatalogGenerator.bat"
    ...
    Write-Host ""
    Write-Host "Alternative: Use the batch file (FamilyCatalogGenerator.bat) directly." -ForegroundColor Yellow
